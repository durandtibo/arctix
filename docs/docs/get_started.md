...
To make the package as slim as possible, only the packages required to use `arctix` are installed.
...
```shell
pip install arctix numpy
```

## Installing from source

To install `arctix` from source, you can follow the steps below. First, you will need to
...
```shell
git clone git@github.com:durandtibo/arctix.git
```
...
```shell
conda activate arctix
```

This example uses `conda` to create a virtual environment, but you can use other tools or
configurations. Then, you should install the required package to use `arctix` with the following
command:
